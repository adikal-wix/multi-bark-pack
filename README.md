...
You're running `node server.js` inside a Claude Code session. Run it in a regular terminal instead.

### Server crashes and doesn't restart

`yarn start` uses `start.sh` which only auto-restarts on clean exit (code 0). If the process crashes (non-zero exit), check the terminal output for the error and run `yarn start` again.
...
```
multi-bark-pack/
  server.js           Main server: routing, commands, agent lifecycle
  stream-display.js   Parses streaming output, writes progress files
  start.sh            Auto-restart wrapper script
  adapters/           Chat platform adapters
    whatsapp.js       WhatsApp adapter (whatsapp-web.js)
    telegram.js       Telegram adapter (Bot API via fetch)
