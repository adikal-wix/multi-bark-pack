...
# Usage: ./install-backends.sh [--yes] [--skip-auth]
...
read_env_var() {
    ...
    if [ -f "$SCRIPT_DIR/.env" ]; then
        grep -E "^${var}=" "$SCRIPT_DIR/.env" 2>/dev/null | head -1 | cut -d= -f2-
    fi
}
...
echo -e "  Next steps:"
echo ""
if [ ! -f "$SCRIPT_DIR/.env" ] || ! grep -q "TELEGRAM_TOKEN\|WA_ENABLED=true\|SLACK_BOT_TOKEN" "$SCRIPT_DIR/.env" 2>/dev/null; then
