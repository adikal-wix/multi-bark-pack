...
if [ "$WIX_MODE" = true ]; then
    ...
    if [ -f "$SCRIPT_DIR/scripts/use-wix-registry.sh" ]; then
        bash "$SCRIPT_DIR/scripts/use-wix-registry.sh"
        ok "Wix registry lock files restored"
    else
        fail "scripts/use-wix-registry.sh not found"
        info "Expected at: $SCRIPT_DIR/scripts/use-wix-registry.sh"
