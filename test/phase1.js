...
const createClaudeCodeBackend = require('../backends/claude-code');
...
test('claude-code buildCommand uses --resume for follow-up', () => {
    ...
    const result = backend.buildCommand({
        promptFile: '/tmp/test.prompt',
        sessionId: 'test-session-id',
        isResume: true,  // This is a follow-up
        model: 'haiku',
        systemPromptFile: '/tmp/test.sysprompt',
        streamParserScript: '/tmp/stream-display.js',
        agentId: 'abc123',
...
const streamParsers = require('../stream-parsers');
const claudeParser = require('../stream-parsers/claude');
...
(async () => {
    ...
    await testAsync('backends.list returns backend info', async () => {
        ...
        assert(list.length > 0, 'should have at least one backend');

        const claude = list.find(b => b.name === 'claude-code');
